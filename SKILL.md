...
目标：15k 中文字 / 11 模块全 / 核心争议专章 ≥ 5 条争议 / 催化剂日历 ≥ 3 条 / 5 XML schema 全 / DS-3 以下占比 < 15%。

### Phase 3 · 质检 + 沉淀

跑 `scripts/qa_check.py <报告路径>` 校验中文字数、核心争议专章（须含 ≥ 5 条争议）、XML schema 数（须 5）、模块数（须 11）。

然后简报回用户：评级 / 目标价 / 置信度 / 关键 gap / 字数。若字数不达标或核心争议专章缺失，回 Phase 2 补。
